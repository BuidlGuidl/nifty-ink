 ...
 * with our candidate fix for upstream issue #5:
 ...
 * Boundary events also leave active touch/captured strokes intact; only an
 * actual release commits them. Unexpected capture loss discards the stroke.
 */
import {
  useCallback,
  useId,
...
export function DrawSurface({
  ...
  return (
    <svg
      ...
      onPointerLeave={(e) => {
        setHover(null);
        // A boundary event is not a finger lift. In particular, touch capture
        // and SVG hit-target changes can produce these during a live drag.
        // Keep collecting until pointerup/cancel (or loss of capture).
        // Preserve the mouse fallback when capture is unavailable.
