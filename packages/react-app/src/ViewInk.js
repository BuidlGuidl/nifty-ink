...
export default function ViewInk(props) {
  ...
  useEffect(() => {
    ...
    dataRaw && dataRaw.ink
      ? setInkTokenTransfers(dataRaw.ink.tokenTransfers)
      : console.log();
    ...
    if (
      ...
      setMintFlow(
        <Popover content={mintInkForm} title="Mint">
          <Button type="secondary" loading={minting}>
            <SendOutlined /> Mint
          </Button>
        </Popover>
      );
  ...
  let mint = async values => {
    ...
    let getSignatureArgs = [
      "0x19",
      "0x00",
      require("./contracts/NiftyToken.address.js"),
      values["to"],
      hash,
    ...
    const bytecode = await props.transactionConfig.current.localProvider.getCode(
      values["to"]
    );
    ...
    <Popover
      ...
      content={
        <div>
          ...
          />
          {copyWord === process.env.REACT_APP_COPY_WORD && (
            <Button
              onClick={() => {
              ...
              <Link to={`/artist/${data.ink.artist.id}`}>
                ...
                <Typography>
                  <span style={{ verticalAlign: "middle", fontSize: 16 }}>
                    {data.ink.createdAt &&
                      new Date(
                        parseInt(data.ink.createdAt) * 1000
          ...
          >
            <ul style={{ listStyle: "none", padding: "5px", margin: "0" }}>
              ...
              >
                ...
                <span
                  style={{ flexBasis: "8%", flexGrow: "1", fontWeight: "bold" }}
                >
                  Price
                </span>
                <span
            ...
            {inkTokenTransfers.map((transfer, transferIndex) => (
              ...
              >
                ...
                <span
                  ...
                  className="token-transfer-table-address"
                >
                  {transfer.from.id ===
                ...
                <span style={{ flexBasis: "8%", flexGrow: "1" }}>
                  {transfer.sale && transfer.sale.price
                    ? "$" + (parseInt(transfer.sale.price) / 1e18).toFixed(2)
                    : "-"}
                </span>
                <span style={{ flexBasis: "12%", flexGrow: "1" }}>
                  {transfer.createdAt &&
                    new Date(
                      parseInt(transfer.createdAt) * 1000
                    ).toLocaleString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric"
              ...
              <CanvasDraw
                ...
                onChange={() => {
                  try {
                    ...
                    if (
                      drawnLines.current >= totalLines.current &&
                      canvasState !== "ready"
                    ) {
