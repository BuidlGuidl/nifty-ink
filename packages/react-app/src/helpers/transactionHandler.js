    ...
    Modal.info({
      ...
      content: (
        <Space direction="vertical">
          ...
          {ethers.Signer.isSigner(c["injectedProvider"]) ? null : (
    ...
    if (parseFloat(ethers.utils.formatEther(balance)) > 0.001) {
      if (injectedNetwork.chainId === localNetwork.chainId) {
        ...
        let contract = new ethers.Contract(
          contractAddress,
          contractAbi,
          ethers.Signer.isSigner(c["injectedProvider"])
            ? c["injectedProvider"]
        ...
        let result;
        if (
          c["injectedProvider"].provider &&
          c["injectedProvider"].provider.wc
        ) {
          let populatedTransaction = await contract.populateTransaction[
            c["regularFunction"]
          ...
          result = await c["injectedProvider"].send("eth_sendTransaction", [
            populatedTransaction
          ]);
        } else {
          result = await contract[c["regularFunction"]](
            ...c["regularFunctionArgs"],
            metaData
          );
        }
        ...
        if (
          c["injectedProvider"].connection &&
          c["injectedProvider"].connection.url === "metamask"
        ) {
      ...
      if (
        ...
        if (
          ...
          console.log(populatedTransaction);
          result = await c["injectedGsnSigner"].send("eth_sendTransaction", [
            populatedTransaction
          ]);
        } else {
          result = await contract[c["regularFunction"]](
