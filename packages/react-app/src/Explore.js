...
export default function ForSale(props) {
  ...
  let [orderDirection, setOrderDirection] = useState(
  ...
  let [endDate, setEndDate] = useState(
    searchParams.has("endDate") ? dayjs(searchParams.get("endDate")) : dayjs()
  );

  let [forSale, setForSale] = useState(
    searchParams.get("forSale") || "all-inks"
  );
        ...
        <ul style={{ padding: 0, textAlign: "center", listStyle: "none" }}>
          {inks
            ? Object.keys(inks)
                .sort(
                  (a, b) =>
                    inks[orderDirection == "desc" ? b : a][orderBy] -
                    inks[orderDirection == "desc" ? a : b][orderBy]
                )
                .map(ink => {
                  let likeInfo =
                    likes.length > 0 &&
                    likes.find(
                      element => element.inkNumber === inks[ink].inkNumber
                    );
                  return (
                    <li
                      key={inks[ink].id}
                            ...
                            >
                              ...
                              <div style={{ marginLeft: 10, marginRight: 10 }}>
                                <LikeButton
                                  ...
                                  hasLiked={
                                    (likeInfo && likeInfo.likes.length > 0) ||
                                    false
                                  }
                                  marginBottom={"0px"}
                                />
